"""
BALLDONTLIE NCAAF (College Football) connector.

Confirmed against https://www.balldontlie.io/openapi/ncaaf.yml (the
...
- The away side is called "visitor_team", not "away_team" (same as NBA)
- Team objects use "full_name" and "abbreviation"
- status_state present, shared cross-sport enum
- The dates[] parameter is explicitly documented with that exact name
  for this sport (unlike NHL's spec, which left it ambiguous)
"""
...
class BallDontLieNCAAFConnector(BallDontLieBaseConnector):
    provider_name = "balldontlie_ncaaf"
    ...
    def _map_game(raw: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        away = raw.get("visitor_team")  # NCAAF calls it "visitor_team"
